...
# Mutable wrapper around a Mysql2::Client.
# All DB calls in this file go through db.query(sql, *params).
# On a dropped-connection error it closes the dead socket, opens a fresh
# one, and retries the statement exactly once.
DbHandle = Struct.new(:client) do
  def query(sql, *params)
    ...
    begin
      ...
      if attempt == 1 && RECONNECT_ERRORS.include?(e.errno)
        log("DbHandle: connection lost (#{e.errno}), reconnecting")
        client.close rescue nil
        self.client = db_connect
        retry
      end
      log("DbHandle: DB error #{e.errno}: #{e.message}")
      nil
    end
  end
...
# Convenience: open, run one query, close.
def db_query_once(sql, *params)
  db = DbHandle.new(db_connect)
  db.query(sql, *params)
ensure
  db&.close
end
...
def ChildProc(newsocket, debug_flag)
  ...
  db = DbHandle.new(db_connect)
  db.query("INSERT INTO HIDSoftLog (device, message, type) VALUES (?, ?, ?)",
  ...
  while i < 240
    if newsocket.ready?
      ...
      while flag == 1
        ...
        # Heartbeat.
        when "1080"
          db.query("INSERT INTO HIDActive SET mac = ?, ip = ?, timestamp = NOW() " \
        ...
        when "1073"
          ...
          len = fields[1].to_i
          Thread.new do
            thread_db = DbHandle.new(db_connect)
            begin
        ...
        when "1052"
          if fields[2] == "0"
            ...
            db.query("UPDATE HIDReaders SET changeover = CURRENT_TIMESTAMP WHERE mac = ?", st.readermac)
            db.query("UPDATE HIDReaders SET manual_open = '0' WHERE mac = ?", st.readermac)
          else
            db.query("INSERT INTO HIDSoftLog (device, message, type) VALUES (?, ?, ?)",
                     st.clientip, "LogMsg 1052: DB Changeover failed with #{fields[2]}, restarting push", '1')
            db.query("UPDATE HIDReaders SET manual_open = '200' WHERE mac = ?", st.readermac)
          end
          flag = 0
...
def checkActiveNeed(socket, st, db)
  ...
  if (row[3].to_i - row[1].to_i) > ten_minutes
    ...
    db.query("UPDATE HIDReaders SET manual_open = '240' WHERE mac = ?", st.readermac)
    return
  end

  if (row[3].to_i - row[2].to_i) > ten_minutes
    log("IdentDB is not within 10 minutes of Changeover")
    db.query("UPDATE HIDReaders SET manual_open = '250' WHERE mac = ?", st.readermac)
    return
  end
...
def chkManualOverride(socket, st, db)
  res = db.query("SELECT manual_open, cmd, ftp, failcount FROM HIDReaders WHERE mac = ?",
                 st.readermac)
  if res.nil? || res.empty?
    db.query("INSERT INTO HIDReaders (mac, groups, comment, manual_open, cmd) VALUES (?, '35', 'Reader-#{st.readermac}', '0', '100')",
             st.readermac)
    return
  end
  ...
  when 100
    ...
    SendDBChange(socket, st, db)
    db.query("UPDATE HIDReaders SET cmd = '0' WHERE mac = ?", st.readermac)
  end

  case manual_open
  when 1
    st.stack << "0094;0016;0;7;2;"
    db.query("INSERT INTO HIDLog (reader, taskcode, message) VALUES (?, 'GRANT', 'Manual Admit')",
             st.readermac)
    db.query("UPDATE HIDReaders SET manual_open = '0' WHERE mac = ?", st.readermac)
  ...
  when 187
    ...
    db.query("INSERT INTO HIDSoftLog (device, message) VALUES (?, 'Reader Reboot requested')",
             st.readermac)
  ...
  when 200
    ...
    if ftp == 1 || failcount == 5
      ...
      db.query("UPDATE HIDReaders SET manual_open = '500' WHERE mac = ?", st.readermac)
    else
      db.query("INSERT INTO HIDSoftLog (device, message, type) VALUES (?, ?, ?)",
...
def ChkTimeSync(socket, st, db)
  res = db.query("SELECT UNIX_TIMESTAMP(lasttimesync) as ts FROM HIDReaders WHERE mac = ?",
                 st.readermac)
  return if res.nil? || res.empty?
  ...
  if threshold > db_time
    SetTime(socket, st, db)
    db.query("UPDATE HIDReaders SET lasttimesync = NOW() WHERE mac = ?", st.readermac)
  end
end
...
def ftpReader(socket, st, db)
  ...
  db.query("UPDATE HIDReaders SET lasttimesync = ? WHERE mac = ?", timeinsert, st.readermac)
end
...
def newReaderSetup(socket, st, db)
  ...
  message = recv_expect(socket, "1030", st, db, "Readers final chunk")
  unless message
    db.query("INSERT INTO HIDSoftLog (device, message, type) VALUES (?, ?, ?)",
...
def ParseCard(msg, socket, st, db)
  ...
  res = db.query("SELECT groupid, id FROM HIDCards " \
                 "WHERE cardnum = ? " \
  ...
  if res.nil? || res.empty?
    ...
    db.query("INSERT INTO HIDLog (reader, taskcode, message) VALUES (?, 'DENY-UNKWN', ?)",
  ...
  reader_res = db.query("SELECT groups FROM HIDReaders WHERE mac = ?", mac)
  ...
  if groups.include?(card_row["groupid"].to_s)
    db.query("INSERT INTO HIDLog (reader, taskcode, message) VALUES (?, 'GRANT', ?)",
             st.readermac, card_num)
    db.query("INSERT INTO HIDSoftLog (device, message, type) VALUES (?, ?, ?)",
             st.clientip, "Card #{card_num} granted", '2')
    safe_write(socket, "0094;0016;0;7;2;", st)
  else
    db.query("INSERT INTO HIDLog (reader, taskcode, message) VALUES (?, 'DENY-NOACC', ?)",
             st.readermac, card_num)
    db.query("INSERT INTO HIDSoftLog (device, message, type) VALUES (?, ?, ?)",
...
def parseCheckCard(msg, socket, st, db)
  ...
  res = db.query("SELECT groupid, contact_id, id FROM HIDCards " \
  ...
  reader_res = db.query("SELECT groups FROM HIDReaders WHERE mac = ?", st.readermac)
    ...
    if time_field && date_field
      ...
      if info0 == "1060"
        if info4 == "2" && info2 == "1"
          ...
          db.query("INSERT INTO HIDLog (reader, taskcode, message, timestamp) VALUES (?, ?, ?, ?)",
                   st.readermac, task_code, info11, timestamp)
        elsif info3 == "4"
          if info9 == "901"
            if info10 == "1"
              db.query("INSERT INTO HIDLog (reader, taskcode, message, timestamp) VALUES (?, 'ALARM', 'DOOR HAS BEEN FORCED', ?)",
                       st.readermac, timestamp)
              db.query("UPDATE HIDAlarm SET alarm = '1' WHERE id = '1'")
            ...
            if info10 == "1"
              db.query("INSERT INTO HIDLog (reader, taskcode, message, timestamp) VALUES (?, 'HELD', 'DOOR HAS BEEN HELD', ?)",
                       st.readermac, timestamp)
              st.stack << "0094;0017;0;11;0;"
            elsif info10 == "0"
              st.stack << "0094;0017;0;11;1;"
...
# ─────────────────────────────────────────────────────────────────────────────
def send_file_to_reader(socket, st, db, remote_path, local_path, label)
  ...
  message = recv_expect(socket, "1030", st, db, "#{label} VERTX_SEND_FILE")
  unless message
    db.query("INSERT INTO HIDSoftLog (device, message, type) VALUES (?, ?, ?)",
  ...
  if fields[0] == "9980"
    ...
    sleep 5
    safe_write(socket, cmd, st)
    message = recv_expect(socket, "1030", st, db, "#{label} RETRY")
  ...
  while data.size > 4084
    ...
    message = recv_expect(socket, "1030", st, db, "#{label} chunk #{chunk_count}")
    unless message
      db.query("INSERT INTO HIDSoftLog (device, message, type) VALUES (?, ?, ?)",
  ...
  sleep 2
  message = recv_expect(socket, "1030", st, db, "#{label} VERTX_SEND_DATA_END")
...
def SendAccessDB(socket, st, db)
  ...
  send_file_to_reader(socket, st, db,
...
def transferDB(socket, st, db)
  res = db.query("SELECT ftp, failcount FROM HIDReaders WHERE mac = ?", st.readermac)
  if res.nil? || res.empty?
    db.query("INSERT INTO HIDSoftLog (device, message, type) VALUES (?, ?, ?)",
  ...
  if ftp == 1
    ...
    db.query("UPDATE HIDReaders SET ftp = '1' WHERE mac = ?", st.readermac)
    ftpReader(socket, st, db)
  else
    SendAccessDB(socket, st, db)
    SendIdentDB(socket, st, db)
